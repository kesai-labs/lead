...
def compute_rfs(
    ...
    rater_specified_trajectories: list[list[jt.Float[npt.NDArray, "T 2"]]],  # [B 3 T 2] where T is not homogenous
    ...
    Args:
        prediction_trajectories: [batch, time steps, 2] numpy array of predicted trajectories.
        rater_specified_trajectories: List of [3, time steps, 2] numpy array of rater specified trajectories.
        rater_scores: [batch, 3] numpy array of scores
        initial_speed: [batch] numpy array of speed
    Returns:
        The rater feedback score (float value typically between 0 and 1).
    """
    ...
    rater_feedback_metrics = get_rater_feedback_score(
        ...
        rater_scores,
        initial_speed,
        frequency=4,  # Default is 4 Hz
        length_seconds=5,  # Default predict 5 seconds
        output_trust_region_visualization=False,
    )
    if detailed_output:
        return rater_feedback_metrics
    return rater_feedback_metrics["rater_feedback_score"]
