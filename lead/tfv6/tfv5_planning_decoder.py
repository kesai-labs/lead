...
class TFv5PlanningDecoder(nn.Module):
    ...
    def __init__(self, input_bev_channels: int, config: TrainingConfig, device: torch.device):
        ...
        self.query = nn.Parameter(
            torch.zeros(
                1,
                self.config.num_route_points_prediction + self.config.num_way_points_prediction + 1,
        ...
        self.route_gru = torch.nn.GRU(
            input_size=config.transfuser_token_dim, hidden_size=config.gru_hidden_size, batch_first=True
        )
        self.route_decoder = nn.Sequential(nn.Linear(config.gru_hidden_size, 2))

        self.wp_gru = torch.nn.GRU(input_size=config.transfuser_token_dim, hidden_size=config.gru_hidden_size, batch_first=True)
    ...
    def forward(
        ...
        """
        context_tokens = self.planning_context_encoder(bev_features=bev_features, data=data, log=log)
        ...
        target_point = data["target_point"].to(device=self.device, dtype=self.config.torch_float_type, non_blocking=True)
        ...
        return route, waypoints, target_speed_dist, decode_two_hot(torch.softmax(target_speed_dist, dim=-1), self.config), None

    @beartype
    def compute_loss(self, predictions, data: dict, loss: dict, log: dict):
        waypoints_label = data["future_waypoints"].to(self.device, dtype=self.config.torch_float_type, non_blocking=True)[
        ...
        loss.update(
            {
                "loss_tf": F.l1_loss(predictions.pred_future_waypoints, waypoints_label, reduction="none").mean(),
                "loss_tfpp_speed": F.cross_entropy(predictions.pred_target_speed_distribution, target_speed_distribution),
                "loss_tfpp": F.l1_loss(predictions.pred_route, route_label),
            }
        )

        if "iteration" in data and ((data["iteration"] + 1) % self.config.wandb_log_scalars_frequency) == 0:
            target_speed_labels = decode_two_hot(target_speed_distribution, self.config)
            log.update(
                {
                    "metric/route_ade": t_u.average_displacement_error(predictions.pred_route, route_label),
                    "metric/route_fde": t_u.final_displacement_error(predictions.pred_route, route_label),
                    ...
                    "metric/waypoints_ade": t_u.average_displacement_error(predictions.pred_future_waypoints, waypoints_label),
                    "metric/waypoints_fde": t_u.final_displacement_error(predictions.pred_future_waypoints, waypoints_label),
                }
            )
...
@beartype
def decode_two_hot(two_hot_label: jt.Float[torch.Tensor, "B C"], config: TrainingConfig) -> jt.Float[torch.Tensor, " B"]:
    ...
    """
    assert torch.all((0.0 <= two_hot_label) & (two_hot_label <= 1.0)), "Two-hot labels must be between 0 and 1"
    assert torch.allclose(
        torch.sum(two_hot_label, dim=-1),
        torch.ones(two_hot_label.shape[0], device=two_hot_label.device, dtype=two_hot_label.dtype),
    ), "Two-hot labels must sum to 1"

    target_speeds = torch.tensor(config.target_speeds, device=two_hot_label.device, dtype=two_hot_label.dtype).unsqueeze(0)
...
@beartype
def encode_two_hot(
    scalar_speed: jt.Float[torch.Tensor, " B"], brake: jt.Bool[torch.Tensor, " B"], config: TrainingConfig
) -> jt.Float[torch.Tensor, "B C"]:
    assert all(scalar_speed >= 0.0)
    target_speeds = torch.tensor(config.target_speeds, dtype=scalar_speed.dtype, device=scalar_speed.device)
    labels = torch.zeros(len(scalar_speed), len(target_speeds), dtype=scalar_speed.dtype, device=scalar_speed.device)
    labels[brake, 0] = 1.0
    non_brake = ~brake
...
class PlanningContextEncoder(nn.Module):
    ...
    def __init__(self, config: TrainingConfig, input_bev_channels: int, device: torch.device):
        ...
        if self.config.use_discrete_command:
            self.num_status_tokens += 1
            self.command_encoder = nn.Sequential(nn.Linear(6, self.config.transfuser_token_dim))
